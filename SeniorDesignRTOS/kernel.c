...
//                                        0x01060019
    ...
    NVIC_MPU_ATTR_R = 0x01060019;
    //region 6
    //RWX, used for turing board global variables
    NVIC_MPU_BASE_R = 0x20007C16; // region 6
    NVIC_MPU_ATTR_R |= 0x03020013;
    /*
     * XN = 0 (execute enabled)
     * AP = 0x3 (privileged RW/ unprivileged RW)
     * TEX = 0
     * S = 0
     * C = 1
     * B = 0
     * SRD = 0
     * Size = 0x9 - > 2^(size + 1) = 1024 -> 1kB
     */
}
...
//void setSrd(uint32_t srd)
//{
//    disableSubRegion(2, (srd >> 0) & 0x000000FF);
//    disableSubRegion(3, (srd >> 8) & 0x000000FF);
//    disableSubRegion(4, (srd >> 16) & 0x000000FF);
//    disableSubRegion(5, (srd >> 24) & 0x000000FF);
//}

void disableSubRegion(uint8_t regionNumber, uint32_t SRD) {
    NVIC_MPU_NUMBER_R = regionNumber;
    NVIC_MPU_ATTR_R &= ~(SRD_MASK); //0x0000FF00
    NVIC_MPU_ATTR_R |= SRD; //<< 8;
}
