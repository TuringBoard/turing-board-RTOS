...
#define SVC_PS 17
#define SVC_PI 18
#define SVC_RESTARTTHREAD 19
#define SVC_DESTROYTHREAD 20
#define SVC_UARTCOMMUNICATION 21
...
//    NVIC_ST_RELOAD_R = 39999;  // = systick isr rate/ system clock  = 1 ms/ 25 ns
    NVIC_ST_RELOAD_R = 1999;  // = systick isr rate/ system clock  = 1 ms/ 25 ns
...
//                current.time = processTimes[i];
    ...
    if(N == SVC_RESTARTTHREAD) {
        _fn fn = (_fn)getR0FromPSP(psp);
        uint8_t i = 0;
        uint32_t n = (uint32_t)fn;
        uint32_t m = (uint32_t)tcb[i].pid;
        for(; i < MAX_TASKS; i++) {
            if(tcb[i].pid == fn) {
                restartThread((_fn)tcb[i].pid);
                break;
...
//    if(N == SVC_UARTCOMMUNICATION) {
//        //check if we are receiving or sending
//        if(sendUartFlag)
//            uartSendOutput();
//        else if(receiveUartFlag)
//            uartReceiveInput();
//        else
//            return;
//    }
...
void postSemaphore(uint8_t index) {
    __asm(" SVC #5");
}
void callRestartThread(_fn fn) {
    __asm(" SVC #19");
}
void callDestroyThread(_fn fn) {
    __asm(" SVC #20");
}
...
void readKeys()
{
    ...
    while(true)
    {
        ...
        {
//            post(flashReq);
            postSemaphore((uint8_t)flashReq);
            RED_LED = 0;
        }
        if ((buttons & 4) != 0)
        {
//            restartThread(flash4Hz);
            callRestartThread(flash4Hz);
        }
        if ((buttons & 8) != 0)
        {
//            destroyThread(flash4Hz);
            callDestroyThread(flash4Hz);
        }
        if ((buttons & 16) != 0)
        {
...
int main(void)
{
    ...
    ok = createThread(uartCommunication, "UartCommunication", 2, 1024);
    ok = createThread(turningMechanism, "TurningMechanism", 2, 1024);
    ok = createThread(ledThread, "LedThread", 2, 1024);
